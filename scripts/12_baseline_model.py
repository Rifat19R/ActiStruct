...
With only 12 candidate structures (3 per system), GPR is the appropriate
baseline because:
  - It produces calibrated uncertainty estimates natively
  - It does not overfit on tiny datasets by construction (exact Bayesian
    posterior, no free lunch needed)
  - Its lengthscale hyperparameters are optimised by marginal likelihood
    maximisation, not cross-validation (too few samples for reliable CV)

CRITICAL DISCLAIMER: With 12 training points across 4 systems (3 per system),
no performance metric from this model is statistically meaningful. The leave-
one-system-out (LOSO) cross-validation implemented here evaluates generalization
to unseen *systems*, not unseen *configurations*. The LOSO results are reported
only to demonstrate the acquisition/uncertainty interface, not to make predictive
claims. The uncertainty values are used as inputs to actistruct's acquisition
functions — their relative magnitude matters, not their absolute accuracy.

Reads:
  data/features/features_v0.1.csv  (via scripts/11_dataset_loader.py)

Writes:
  data/models/baseline_gp_v0.1.json   (LOSO CV results + model metadata)
  reports/baseline_model_report_v0.1.md

Usage:
    python scripts/12_baseline_model.py              # full LOSO CV + report
...
def main() -> None:
    ...
    if args.dry_run:
        ds = load_dataset(target=args.target, candidates_only=True)
    ...
    ranked = rank_candidates_by_acquisition(full_ds, args.feature_set)
    ...
    build_report(fold_results, ranked, args.target, report_path)
