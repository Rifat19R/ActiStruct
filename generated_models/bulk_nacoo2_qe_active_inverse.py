"""Active learning + inverse design for layered NaCoO2 from a CIF cell."""
...
_NACOO2_SYMBOLS = ["Na"] * 3 + ["Co"] * 3 + ["O"] * 6
_NACOO2_SCALED_POSITIONS = [
...
def build_bulk_nacoo2(a: float, c_over_a: float) -> Atoms:
    a = float(a)
    c = float(c_over_a) * a
    cell = [
        (a, 0.0, 0.0),
    ...
    atoms = Atoms(_NACOO2_SYMBOLS, cell=cell, pbc=True)
    atoms.set_scaled_positions(_NACOO2_SCALED_POSITIONS)
    atoms.wrap()
    return atoms
...
SYSTEM = ActiveSystem(
    key='bulk_nacoo2',
    title='Layered NaCoO2 Model',
    builder=build_bulk_nacoo2,
    variables=(
    ...
    notes="CIF-derived Na3Co3O6 hexagonal cell; fractional coordinates fixed while a and c/a vary.",
)
