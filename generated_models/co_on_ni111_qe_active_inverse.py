"""Active learning + inverse design for CO on Ni(111)."""
...
def build_co_on_ni111(height: float, shift: float):
    return build_adsorbate('CO', 'Ni', height, shift)


SYSTEM = ActiveSystem(
    key='co_on_ni111',
    title='CO on Ni(111)',
    builder=build_co_on_ni111,
    variables=(
        Variable("height", 1.72, 1.92, (1.74, 1.82, 1.9, 1.78, 1.86)),
        Variable("shift", 0.0, 0.15, (0.0, 0.04, 0.08, 0.12, 0.15)),
    ),
    pseudopotentials={'C': 'C.pbe-n-kjpaw_psl.1.0.0.UPF', 'O': 'O.pbe-n-kjpaw_psl.0.1.UPF', 'Ni': 'ni_pbe_v1.4.uspp.F.UPF'},
    ecutwfc=80.0,
    ecutrho=640.0,
    kpts=(6, 6, 1),
    smearing='mv',
