...
| Clean Ti3C2-O slab is independently regenerated and DFT-relaxed to convergence (E = -26041.297821 eV, JOB DONE, fmax=0.028 eV/A). | `data/structures/ti3c2_o/ti3c2_o_slab_relaxed.traj`; `data/evidence/ti3c2_o_clean_slab_lf/` | `python -m examples.manual_qe.build_ti3c2_o_slab` | Absolute energy is not comparable to any earlier, differently-built structure -- only self-consistent differences computed against this retained structure are meaningful. |
| A live 3-track LF active-learning campaign (GNN-embedding GP, plain GP on periodic features, random baseline) has completed under the frozen protocol; the plain-GP track found the best near-thermoneutral site (`\|DeltaG_H\|=0.0020` eV) of the three, in fewer DFT calls and less wall time than random. | `docs/TI3C2O_LF_CAMPAIGN_RESULTS.md`; `outputs/campaigns/ti3c2_o_lf_campaign.jsonl`; `outputs/campaigns/ti3c2_o_lf_campaign_plain_gp_rerun_amend5.jsonl` | `FIDELITY=low python -m examples.manual_qe.run_ti3c2_o_al_loop` | Single system, single seed set, single acquisition budget. Does not support "GP beats GNN" or "engineered surrogates beat random search" claims outside this test. |
...
| LF ranking transfers reliably to HF. | Not available -- attempted and deferred, see `docs/HF_VALIDATION_STATUS.md` and `data/evidence/ti3c2_o_hf_ranking_validation_interrupted/`. | Not available. | Requires representative LF/HF comparison, rank correlation, and ranking-stability report; HF clean-slab reference did not complete in 3 attempts. |
