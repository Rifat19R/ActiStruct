...
- **`ibrav=0` -> `ibrav=1`/`celldm(1)`, plus `trust_radius_min` lowered to `1.0e-6` bohr (2026-06-29):** a real cr_co6 relax got `bfgs failed after 10 scf cycles and 7 bfgs steps, convergence not achieved` - forces had already dropped to ~0.0004 Ry/au (close to `forc_conv_thr=1e-4`) but the BFGS trust radius collapsed below QE's default `trust_radius_min` floor. `ibrav=0` (flagged by QE itself as `DISCOURAGED`) used a generic `CELL_PARAMETERS` matrix that can carry tiny floating-point asymmetries showing up as force noise near a symmetric stationary point; switched to the correct `ibrav=1` for `cell_type: cubic_supercell`. Lowering `trust_radius_min` gives BFGS room to keep refining instead of aborting. Applied to all 4 systems' QE input generation in `scripts/06_build_qe_inputs.py`, re-verified with smoke tests (no parse errors, same RAM/symmetry as before). Full re-relax to confirm actual convergence is Rifat's to run.
