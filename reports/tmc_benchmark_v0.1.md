# TMC Reliability Benchmark v0.1

*ActiStruct-nebwalk Transition-Metal Complex Benchmark — Phase 2 complete.*
...
| Functional | PBE (GGA) |
| Pseudopotentials | SSSP efficiency tier v1.3.0 (Fe, C, H, O); Ni/Cr from same folder (naming-convention unverified — see §8) |
...
| Ni/Cr pseudopotential tier confirmed | ❌ Naming convention differs from SSSP-efficiency pattern for Fe/C/H/O |
| `negative_rho` warnings | ⚠️ Present in all 16 calculations; magnitudes (<0.021 e/bohr³) are typical for plane-wave PBE and not a correctness issue |
...
| Dispersion correction | ❌ Not applied (PBE-D3 or similar recommended for ferrocene's weak Cp–Fe bonding before publication) |
...
└── reports/
    ...
    └── tmc_benchmark_v0.1.md            — this document
```
...
2. **Ni/Cr pseudopotential audit** — confirm SSSP efficiency tier vs what is
   actually in `D:/Rifat_kh/SSSP_1.3.0_PBE_efficiency/`.
